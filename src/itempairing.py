...
def _calculate_description_similarity(desc1, desc2):
    ...
    if not desc1 and not desc2: return 1.0
    ...
    try:
        ...
        logger.error(f"Error calculating description similarity for '{desc1}' vs '{desc2}': {e}", exc_info=False)
        return 0.0

def _calculate_item_id_similarity(id1, id2):
    if not model: return 0.0
    if not id1 and not id2: return 1.0
    if not id1 or not id2: return 0.0
    s_id1, s_id2 = str(id1), str(id2)
    if s_id1 == s_id2: return 1.0
    try:
        embeddings = model.encode([s_id1, s_id2])
        similarity = np.dot(embeddings[0], embeddings[1])
        return float(similarity) if not np.isnan(similarity) else 0.0
    except Exception as e:
        logger.error(f"Error calculating item ID similarity for '{id1}' vs '{id2}': {e}", exc_info=False)
        return 0.0

def _calculate_unit_price_similarity(up1, up2):
    if up1 is None or up2 is None: return 0.0
    try:
        f_up1 = float(up1)
        f_up2 = float(up2)
    except (ValueError, TypeError):
         logger.debug(f"Could not convert unit prices '{up1}', '{up2}' to float for similarity.")
         return 0.0

    if isclose(f_up1, f_up2, rel_tol=1e-5): return 1.0
...
def _calculate_match_score(item_id_sim, desc_sim, price_sim):
    is_match = (item_id_sim >= 0.99) or \
               (item_id_sim > 0.8 and isclose(price_sim, 1.0) and desc_sim > 0.4) or \
               (desc_sim > 0.95 and isclose(price_sim, 1.0) and item_id_sim < 0.5)
...
def find_best_item_match(source_item_data: dict, target_items_data: list[dict]) -> dict | None:
    ...
    source_price = source_item_data.get("unit-price-adjusted", source_item_data.get("unit-price"))
    ...
    for target_item in target_items_data:
        ...
        score, is_potential_match = _calculate_match_score(item_id_sim, desc_sim, price_sim)
    ...
    return max(potential_matches, key=lambda x: x["score"]) if potential_matches else None

def pair_document_items(doc1_items_data: list[dict], doc2_items_data: list[dict]) -> list[dict]:
    if not model:
         logger.error("SentenceTransformer model not available. Cannot perform item pairing.")
         return []

    for item in doc1_items_data: item['matched'] = False
    for item in doc2_items_data: item['matched'] = False
        ...
        if best_match_info:
            ...
            try:
                available_doc1_items.remove(doc1_matched_item)
                 ...
                 logger.warning(f"Item index {doc1_matched_item.get('item_index')} scheduled for removal not found in available pool.")
