...
def get_differing_amounts_severity(amount1: Decimal | float, amount2: Decimal | float) -> DeviationSeverity | None:
    try:
        d_amount1 = Decimal(str(amount1))
        d_amount2 = Decimal(str(amount2))
    except Exception:
         logger.warning(f"Could not convert amounts '{amount1}', '{amount2}' to Decimal for comparison.")
...
def check_itempair_comparison(
        ...
        try:
            if field_name_config == "!quantityToInvoice*unitAmount":
                 if document_kind != DocumentKind.PURCHASE_ORDER:
                     logger.warning(f"Attempted PO amount calculation '{field_name_config}' on non-PO doc type: {document_kind}")
                     values.append(None)
                     continue
                 ...
                 if quant_val is not None and unit_price_val is not None:
                    ...
                    logger.debug(f"Missing quantity ('{quant_val}') or unit price ('{unit_price_val}') for amount calculation in {document_kind}.")
                    value = None
            else:
            ...
            logger.error(f"Failed to process/convert value for {document_kind} field '{field_name_config}' (Raw: '{raw_value}'): {e}", exc_info=False)
            value = None
    ...
    for i, value1 in enumerate(non_empty_values):
        for value2 in non_empty_values[:i]:
            ...
            if comparison.code == "amount-differs" or comparison.code == "unit-amount-differs":
                ...
                if severity_calc is not None:
                    ...
                    final_message = f"{comparison.message} ({value1} vs {value2}, diff: {diff:.2f})"
...
def collect_itempair_deviations(
    ...
    if len(document_kinds) != len(document_item_fields):
        logger.error("Mismatch between number of document kinds and item fields provided.")
        return deviations

    for comparison in FIELD_COMPARISONS:
        if comparison.is_item_field:
             deviation = check_itempair_comparison(comparison, document_kinds, document_item_fields)
             if deviation:
...
if __name__ == "__main__":
    ...
    results = collect_itempair_deviations(
        [DocumentKind.INVOICE, DocumentKind.PURCHASE_ORDER, DocumentKind.DELIVERY_RECEIPT],
        [
            [
