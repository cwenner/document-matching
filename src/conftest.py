...
def pytest_addoption(parser):
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="skip tests requiring a model",
    )
    parser.addoption(
        "--only-model",
        action="store_true",
        default=False,
        help="only run tests requiring a model",
    )
...
def pytest_collection_modifyitems(config, items):
    if config.getoption("--skip-model"):
        skip_model = pytest.mark.skip(
            reason="skipping model tests with --skip-model option"
        )
        for item in items:
    ...
    if config.getoption("--only-model"):
        skip_non_model = pytest.mark.skip(
            reason="skipping non-model tests with --only-model option"
        )
        for item in items:
