...
def run_matching_pipeline(predictor: DocumentPairingPredictor, input_document: dict, historical_documents: list[dict]):
    """
    ...
    """
    logger.info(f"--- Running Pipeline for Document ID: {input_document.get('id', 'N/A')} ---")
    ...
    try:
        ...
        error_report["deviations"] = [{"code": "pipeline-setup-error", "message": f"Failed to record documents: {e}", "severity": "high"}]
    ...
    if pairings:
        ...
        if matched_doc_id not in id2doc:
            ...
            final_report["deviations"] = [{"code": "match-lookup-failed", "message": f"Matched ID {matched_doc_id} not found.", "severity": "high"}]
            ...
            if not doc1_items or not doc2_items:
                ...
                try:
                     ...
                     for raw_pair in matched_item_pairs_raw:
                         ...
                         if item1_fields is None or item2_fields is None:
                              ...
                              try:
                                 ...
                                 item_deviations.append(FieldDeviation(code="deviation-calc-failed", message=str(e_dev), severity=DeviationSeverity.HIGH))
                         ...
                         processed_item_pairs.append(
                             {
                                 ...
                                 "similarities": raw_pair.get("similarities"), # Keep for potential debugging/reporting
                     ...
                     document_deviations.append(FieldDeviation(code="item-processing-failed", message=str(e_item_processing), severity=DeviationSeverity.HIGH))
        ...
        try:
            ...
            final_report = {
                ...
                "details": str(e),
                 "documents": [ # Try to include basic doc info
                     {"kind": input_document.get("kind", "unknown"), "id": input_document.get("id")},
                 ],
                 "labels": ["no-match", "report-generation-error"]
            }

    logger.info(f"--- Pipeline Finished for Document ID: {input_document.get('id', 'N/A')} ---")
    return final_report
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not os.path.exists(model_path):
            alt_path1 = os.path.join(script_dir, "data", "models", "document-pairing-svm.pkl")
        ...
        logger.exception("An unexpected error occurred during predictor initialization.")
        exit(1)
    ...
    final_report = run_matching_pipeline(predictor, input_document, historical_documents)
    ...
    if final_report:
        ...
        try:
            ...
            print(f"Error: Could not display final report.\nReport data: {final_report}")
    else:
