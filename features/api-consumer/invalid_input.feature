...
Feature: Document Matching API - Invalid Input Handling
  ...
  @error_cases @wrong_http_method
  Scenario: Wrong HTTP Method
    Given the document matching service is available
    When I send a GET request to "/"
    Then the response status code should be 405
    And the response body should contain a clear error message
    And the error message should indicate method not allowed
    And the error message should be machine-readable

  @error_cases @null_document
  Scenario: Null Document Field
  ...
  @error_cases @document_not_object
  Scenario: Document Field Not an Object
  ...
  @error_cases @null_candidates
  Scenario: Null Candidate Documents Field
    Given I have a valid primary document
    And I have null candidate documents field
